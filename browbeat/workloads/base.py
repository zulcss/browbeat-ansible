...
class WorkloadBase(object):
    ...
    def workload_logger(self, workload):
        workload_result_dir = os.path.join(
            self.results_path, self.result_dir_ts)
    ...
    def get_time_dict(self, to_time, from_time, benchmark, test_name,
                      workload, status, index_status="disabled"):
    ...
    def dump_report(result_dir, time_stamp):
        with open(
            os.path.join(
                result_dir, time_stamp + '.' + 'report'), 'w') as yaml_file:
    ...
    def display_summary():
        ...
        WorkloadBase.logger.info(
            "Total tests executed:{}".format(WorkloadBase.total_tests))
        WorkloadBase.logger.info(
            "Total tests passed:{}".format(WorkloadBase.success))
        WorkloadBase.logger.info(
            "Total tests failed:{}".format(WorkloadBase.failure))
